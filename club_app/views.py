...
def history(request):
    try:
        ...
        for r in History:
            ...
            if (int(a[5:7]) >= int(b[5:7]) and int(a[8:10]) >= int(b[8:10])):
                at.append(r)
        ...
        if (request.method=="POST"):
            ...
            # print(button)
            at=[]
            Reservation.objects.filter(id=id).update(Reserved=False,Reserved_person="")
            price =Registered_User.objects.get(email=email)
            price= price.Negative_balance
            Registered_User.objects.filter(email=email).update(Negative_balance=price+100)
        ...
        return render(request,"history.html",{"History":History,"at":at})
...
def payment(request):
    email= request.session["email"]
    nprice =Registered_User.objects.get(email=email)
    nprice= nprice.Negative_balance
    global court_id
    global check
    if (request.method=="POST"):
        if (check==1):
            ...
            if (reserved != "False"):
                pfee = int(ofee)/0.7 +nprice
                paym =Payment(email=email,reser_id=Id,original_amount=pfee,paid_amount=ofee,Date=datetime.now())
                paym.save()
                R.save()
                Registered_User.objects.filter(email=email).update(Negative_balance=0)
                snd_mail2(ofee,email)
                snd_mail22(c)
            else:
                pfee = int(ofee) +nprice
                paym =Payment(email=email,reser_id=Id,original_amount=pfee,paid_amount=ofee,Date=datetime.now())
                paym.save()
                R.save()
                Registered_User.objects.filter(email=email).update(Negative_balance=0)
        ...
        if (check==1):
            ...
            if (re != "False"):
                Pay =int(Pay*0.7) +int(nprice)
                return render(request,"payment.html",{"court_id":court_id,"email":email,"payment":Pay,"Start_Time":Start_Time,"End_Time":End_Time,"p":p})
            else:
                return render(request,"payment.html",{"court_id":court_id,"email":email,"payment":Pay+int(nprice),"Start_Time":Start_Time,"End_Time":End_Time,"p":p})
